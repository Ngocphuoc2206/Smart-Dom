...
interface Invoice {
  ...
  tenant: string;
  rentRooms: number;
  invoiceType: "electric" | "water" | "full" | string; // tùy loại hóa đơn
  invoiceAmount: number;
  electricUsage: number;
  waterUsage: number;
  serviceFees: number;
  note: string;
  invoiceDateLimit: string; // ISO date string: yyyy-MM-ddTHH:mm:ss
...
export default function PaymentPage() {
  ...
  const invoice = invoices.find(
    (inv) => inv.userId === user?.idUser && inv.id.toString() === receiptId
  );
  const mockBill = {
    id: invoice?.id ?? 0,
    ...
    dueDate: invoice?.invoiceDateLimit
      ? invoice.invoiceDateLimit.split("T")[0]
      : "",
    details: [
      { item: "Tiền phòng", amount: invoice?.rentRooms ?? 0 },
      {
      ...
      { item: "Phí dịch vụ", amount: invoice?.serviceFees ?? 0 },
      {
        item: "Giảm giá khách hàng thân thiết",
        amount:
          (invoice?.invoiceAmount ?? 0) -
          ((invoice?.rentRooms ?? 0) +
            (invoice?.electricUsage ?? 0) * 3500 +
            (invoice?.waterUsage ?? 0) * 25000 +
            (invoice?.serviceFees ?? 0)),
      },
    ],
  };
  ...
  const handlePayment = async () => {
    ...
    const res = await fetch(
      "https://localhost:7257/api/invoice/payment/internal",
      {
        method: "POST",
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Hóa đơn {translateInvoiceType(mockBill.type)} đã được thanh toán
            thành công.
          </p>
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            <p className="text-sm text-gray-600">
              Mã giao dịch<b>{transactionCode}</b>
            </p>
            <p>
              Trạng thái: <span className="text-green-600">Thành công</span>
            </p>
          </div>
              ...
              <div className="space-y-2">
                {mockBill.details.map((detail, index) => (
                  <div key={index} className="flex justify-between text-sm">
                    ...
                    <span
                      className={
                        detail.amount < 0 ? "text-green-600" : "text-gray-900"
                      }
                    >
                      {detail.amount < 0 ? "-" : ""}
                      {Math.abs(detail.amount).toLocaleString()}đ
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="border-t mt-4 pt-4">
                <div className="flex justify-between text-lg font-bold">
                  ...
                  <span className="text-green-600">
                    {mockBill.amount.toLocaleString()}đ
                  </span>
                </div>
              </div>
            </div>
                ...
                {selectedMethod === "banking" && (
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <div className="text-sm text-blue-800 space-y-1">
                      ...
                      <p>
                        <strong>Chủ tài khoản:</strong> SMARTDORM COMPANY
                      </p>
                      <p>
                        <strong>Nội dung:</strong> THANHTOAN{" "}
                        {mockBill.roomNumber} {mockBill.tenant}
                      </p>
                    </div>
                  </div>
                )}
